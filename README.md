...
[`personality-assessment/`](personality-assessment/) holds a complete behavioural hiring assessment
and its landing page, in the Forward Framework identity — 70 items scored five ways (Big Five,
DISC, Hogan HPI-style, Predictive Index-style drives, Caliper-style competencies with a reasoning
screen). Static HTML/CSS/ES modules, no dependencies and no server; see
[personality-assessment/README.md](personality-assessment/README.md) for the scoring model, the
norm table, and the fair-use guidance.

```bash
cd personality-assessment && python3 -m http.server 8000   # http://localhost:8000
node personality-assessment/build.mjs                      # one self-contained HTML file
```
