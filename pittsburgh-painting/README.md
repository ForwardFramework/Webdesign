...
├─ tools/                   ← font fetcher, guide renderer, preview bundler, deploy zip
├─ preview.html             ← shareable single-file bundle (not deployed)
└─ PLAN.md  CONTENT-CHECKLIST.md  README.md  wrangler.toml
(netlify.toml sits at the repo root, one level up)
```
...
and cannot be told not to — so if the site used `.html` URLs, every internal link would
...
| The drag-and-drop zip | — | `python3 tools/make-zip.py` |
...
The publish directory is **`pittsburgh-painting/dist`** and there is **no build step** —
`dist/` is committed, so CI has nothing to run. `netlify.toml` at the repo root already sets
`base` and `publish`, so nothing needs configuring by hand.

`_headers` (security + cache-control) and `_redirects` (short URLs for yard signs, plus
`.html` → directory fallbacks) live inside `dist/` and are generated by `build.py`, so they
can't drift from the page list. Netlify reads both natively.

### Fastest — drag and drop

```bash
python3 tools/make-zip.py        # writes pittsburgh-painting-site.zip
```

Drop that file on **[app.netlify.com/drop](https://app.netlify.com/drop)**. The site is live
in about thirty seconds, with no Git connection and no tokens. This also *creates* the site,
which is where the Site ID for the automated route below comes from.

### Connect the repo (auto-deploys on push, no secrets)

Netlify → **Add new site** → **Import an existing project** → GitHub →
`ForwardFramework/Webdesign` → pick the branch → **Deploy**.

Leave every build setting alone; `netlify.toml` supplies them:

```toml
[build]
  base    = "pittsburgh-painting"
  publish = "dist"          # resolved relative to base
```

### GitHub Actions (auto-deploy plus pre-flight checks)

`.github/workflows/deploy-netlify.yml` deploys on every push and additionally verifies the two
things a plain Git connection won't:
...
1. Netlify → **User settings → Applications → New access token** → `NETLIFY_AUTH_TOKEN`
2. Your site → **Site configuration → General → Site ID** → `NETLIFY_SITE_ID`
3. GitHub → repo → **Settings → Secrets and variables → Actions** → add both.

`main` publishes production; any other branch gets its own deploy preview. **Run workflow** on
the Actions tab can push the current branch to production without merging.

### After the first deploy

1. **Custom domain** — Site configuration → *Domain management* → add the real domain.
...
Still supported, now manual only: `.github/workflows/deploy-cloudflare.yml` runs from the
Actions tab on demand (needs `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`), and
`wrangler.toml` points `npx wrangler pages deploy` at `dist/`. It no longer runs on push, so
it won't fail builds while Netlify is the live host.
